...
const MobileCard = ({ card, narrative, compact = false, onRetry }: { card: any, narrative: any, compact?: boolean, onRetry?: () => void }) => {
  ...
  return (
    <div className="relative rounded-2xl border border-slate-800 bg-slate-950 p-4 shadow-lg h-full flex flex-col">
      <div className="flex flex-col xl:flex-row xl:items-start xl:justify-between gap-4 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-sm font-bold text-slate-200 uppercase tracking-wide">{card.life_area}</h3>
            {typeof card.ai_confidence === 'number' && (
              <p className="text-[10px] text-slate-500 font-mono">Confidence {card.ai_confidence}%</p>
            )}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between xl:flex-col xl:items-end xl:gap-1">
          <div className="flex items-baseline gap-2">
            <span className="text-2xl sm:text-3xl font-black text-slate-100 tracking-tight">{score.toFixed(1)}%</span>
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              aria-pressed={pinned}
              className={`p-1.5 rounded-lg border transition-colors ${pinned ? 'border-blue-500/40 bg-blue-600/20 text-blue-200' : 'border-slate-800 bg-slate-900 text-slate-400 hover:text-slate-200'} xl:hidden`}
        ...
        <button
          onClick={() => setPinned(!pinned)}
          aria-pressed={pinned}
          className={`hidden xl:flex absolute top-4 right-4 p-2 rounded-lg border transition-colors ${pinned ? 'border-blue-500/40 bg-blue-600/20 text-blue-200' : 'border-slate-800 bg-slate-900 text-slate-400 hover:text-slate-200'}`}
          title={pinned ? 'Pinned' : 'Pin'}
        >
      ...
      <div className="flex flex-wrap md:flex-nowrap items-center gap-2 mb-2">
        {showTimingChip && (
          <span className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-slate-800 bg-slate-900 text-[10px] font-medium text-slate-300 whitespace-nowrap">
        ...
        {base && (base.dasha || base.transit || base.nakshatra) && (
          <span className="flex-shrink-0 inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg border border-slate-800 bg-slate-900 text-[10px] font-mono text-slate-400 whitespace-nowrap">
            <span className="text-slate-500">D</span> {Math.round(base.dasha || 0)}
            <span className="mx-1 opacity-30">|</span>
            <span className="text-slate-500">T</span> {Math.round(base.transit || 0)}
            <span className="mx-1 opacity-30">|</span>
            <span className="text-slate-500">N</span> {Math.round(base.nakshatra || 0)}
          </span>
        )}
      </div>
      {!compact && (
      <div className="mt-4 pt-4 border-t border-slate-800">
        ...
        <p className="text-sm text-slate-300 leading-relaxed">{isError ? (fallbackText || 'Guidance temporarily unavailable') : card.synthesis}</p>
        {showWhy && (
          <div id={`why-${card.life_area}`} className="mt-3 p-3 bg-slate-800/50 border border-slate-700 rounded-lg text-xs text-slate-300">
                ...
                <div className={`px-3 pb-3 ${isOpen ? 'block' : 'hidden'}`}>
                  {key === 'lucky' ? (
                    <div className="flex flex-wrap items-center gap-2 text-[11px] text-slate-300">
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg border border-slate-800 bg-slate-950">
                        <Palette className="w-3 h-3 text-slate-400" /> Color: {lucky.color || '—'}
                      </span>
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg border border-slate-800 bg-slate-950">
                        <Clock className="w-3 h-3 text-slate-400" /> Time: {lucky.time || card?.optimal_action?.timing || '—'}
                      </span>
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg border border-slate-800 bg-slate-950">
                        <Coins className="w-3 h-3 text-slate-400" /> Number: {lucky.number || '—'}
                      </span>
                    </div>
                  ) : key === 'mood' ? (
                    <div className="space-y-2">
                      ...
                      <p className="text-[12px] text-slate-300">{data?.label || 'Mood unavailable'}</p>
                    </div>
                  ) : key === 'dasha' ? (
                     <p className="text-[12px] leading-relaxed text-slate-300">{dashaTheme}</p>
                  ) : key === 'transit' ? (
                     <p className="text-[12px] leading-relaxed text-slate-300">{transitEffect}</p>
                  ) : key === 'nakshatra' ? (
                     <p className="text-[12px] leading-relaxed text-slate-300">{nakshatraTheme}</p>
                  ) : key === 'advice' ? (
                     <p className="text-[12px] leading-relaxed text-slate-300">{advice}</p>
                  ) : (
                    <p className="text-[12px] leading-relaxed text-slate-300">{data?.text}</p>
                  )}
                </div>
              </div>
            );
      ...
      {Array.isArray(narrative.tokens) && narrative.tokens.length > 0 && (
        <div className="mt-4 flex flex-wrap items-center gap-2">
          {narrative.tokens.map((t: string, i: number) => (
            <span
              key={i}
              className="inline-flex px-2 py-1 rounded-lg border border-slate-800 bg-slate-950 text-[10px] text-slate-300"
        ...
        <p className="text-sm text-slate-200">{card.optimal_action?.action}</p>
        <div className="mt-3 flex flex-wrap items-center gap-2">
          <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg border border-slate-800 bg-slate-950 text-xs text-slate-300">
            <Clock className="w-3 h-3 text-slate-400" /> {card.optimal_action?.timing}
          </span>
          {countdown && (
            <span className="inline-flex items-center gap-1 px-2 py-1 rounded-lg border border-slate-800 bg-slate-950 text-xs text-slate-300">
              <Clock className="w-3 h-3 text-slate-400" /> in {countdown}
            </span>
          )}
          {Array.isArray(card.optimal_action?.cta_buttons) && card.optimal_action.cta_buttons.length > 0 ? (
            <>
              <button className="px-3 py-1.5 rounded-lg bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold">
          ...
          <button onClick={handleCopy} className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold inline-flex items-center gap-1">
            <Copy className="w-3 h-3" />
            {copied ? 'Copied' : 'Copy'}
          </button>
          <button onClick={handleShare} className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold inline-flex items-center gap-1">
            <Share2 className="w-3 h-3" />
            Share
          </button>
          <button onClick={() => setShowNote(!showNote)} className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold inline-flex items-center gap-1">
            <Edit className="w-3 h-3" />
            Note
          </button>
          {onRetry && isError && (
            <button onClick={onRetry} className="px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-semibold inline-flex items-center gap-1">
        ...
        {showNote && (
          <div className="mt-3">
            <textarea
              ...
              className="w-full px-3 py-2 rounded-lg border border-slate-800 bg-slate-950 text-sm text-slate-200"
              rows={2}
            />
          </div>
        )}
    ...
    return (
      <div className="w-full space-y-6">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 border-b border-slate-800 pb-4">
          <div className="flex items-center gap-3">
            <h3 className="text-xl font-bold text-slate-100 tracking-tight">Daily Horoscopes</h3>
            <span className="px-2 py-1 rounded-full bg-slate-900 border border-slate-800 text-xs font-medium text-slate-400">
          ...
          <div className="flex items-center gap-3 flex-wrap">
            <button
              ...
              className="inline-flex items-center gap-2 px-3 py-2 rounded-lg border border-slate-800 bg-slate-900/50 hover:bg-slate-900 text-xs font-medium text-slate-300 transition-colors"
            ...
            {onRefresh && (
              <button
                onClick={onRefresh}
                className="inline-flex items-center gap-2 px-3 py-2 bg-blue-600 hover:bg-blue-500 text-white text-xs font-semibold rounded-lg shadow-lg shadow-blue-900/20 transition-all active:scale-95"
              >
        ...
        <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 bg-slate-900/30 p-1 rounded-xl">
          ...
          <div className="flex items-center gap-3 ml-auto w-full sm:w-auto">
            <div className="relative flex-1 sm:flex-none">
              <select
                ...
                className="w-full sm:w-40 appearance-none px-3 py-2 pl-9 rounded-lg border border-slate-800 bg-slate-900 text-xs text-slate-300 focus:outline-none focus:ring-1 focus:ring-blue-500"
              >
        ...
        <div className="flex items-center justify-between px-4 py-3 rounded-xl border border-slate-800 bg-slate-900/50">
